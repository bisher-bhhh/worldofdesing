...
{
    return view('frontend.portfolio-ajax-project-1');
}
...
//    dd($id);
    ...
    public function  gettestProjects(){
        return view('frontend.projects_test');

    }
    public function getAllProjects(Request $request)
    {
        $lang = !empty(session()->get('lang')) ? session()->get('lang') : Language::where('default', 1)->first()->slug;
        $all_work = Works::where('lang', $lang)->paginate(3);
        $all_work_category = WorksCategory::where(['status' => 'publish', 'lang' => $lang])->get();

        if ($request->ajax()) {
            $view = view('frontend.partials.project_list', ['all_work' => $all_work])->render();
            return response()->json([
                'html' => $view,
                'next_page_url' => $all_work->nextPageUrl()
            ]);
        }

        return view('frontend.allprojects')->with([
            'all_work_category' => $all_work_category,
            'all_work' => $all_work
        ]);
    }
...
//        $lang = !empty(session()->get('lang')) ? session()->get('lang') : Language::where('default',1)->first()->slug;
////        $all_work = Works::where('lang',$lang)->take(3)->get();
//        $all_work = Works::where('lang',$lang)->get();
//        $all_work_category = WorksCategory::where(['status'=> 'publish', 'lang' => $lang])->get();
//        return view('frontend.allprojects')->with([
//                'all_work_category' => $all_work_category,
...
//    }
    ...
    public function loadMoreProjects(Request $request)
    {
        ...
        $projects = Works::skip($skip)->take(3)->get(); // Adjust the number as needed
        return response()->json(['projects' => $projects]);
    }
    public function  getServicesProjects(){
    ...
    public function getportfolioproject(Request $request)
    {
        ...
        return view('frontend.portfolio-ajax-project')->with([
            'data' => $work,
        ]);
    }
    public function getSingleProject(Request $request)
    {
        $work = Works::with('images')->findOrFail($request->id);

        return view('frontend.project_details', compact('work'));
